...
### Фиксированный ответ существующего telegram.send_message (0.5.52)

Необязательный config.replyText (по умолчанию пустая строка) используется,
только если вход text не подключён и не передан. Подключённый text имеет
безусловный приоритет, включая пустое/недоставленное значение: fallback не
маскирует неисправный провод. Пустой итоговый текст остаётся ERR_EMPTY_INPUT.
Получатель всё ещё приходит через chat_id, не берётся неявно из payload.
Движок/ID портов не менялись. Старое эхо и клавиатуры сохраняют прежнюю семантику.

Простой UI собирает два обычных узла: message_received.chat_id →
send_message.chat_id, replyText задаётся в форме. При проверке /start
распознаётся общей messageCommand с тем же правилом, что существовало в
server/src/telegram/updates.ts. Это локальный симулятор, не боевая отправка.
