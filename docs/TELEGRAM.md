...
- Конфигурация бота: `POST/GET/PATCH/DELETE
  ...
  нельзя (404).
- Вебхук получает обновления → строгая валидация → запуск **только
  опубликованной (LIVE) версии** проекта тем же движком → отправка
  через серверный транспорт Bot API (подэтапы 5.8B–5.8D).
